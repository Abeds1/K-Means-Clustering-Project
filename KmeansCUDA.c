...
_device_ double distance_gpu(Point a, Point b) {
...
_device_ double atomicAdd_custom(double* address, double val) {
...
_global_ void assignPoints(Point* points, Point* centroids, int* labels, int numPoints, int numCentroids) {
...
_global_ void updateCentroids(Point* points, Point* centroids, int* labels, int* counts, int numPoints, int numCentroids) {
    extern _shared_ char shared_memory[];
